...
Если синхронный запрос (`/api/1c/products`, `/api/1c/prices`, `/api/1c/stocks`, `/api/1c/categories`) не удался по причине недоступности сайта, 1С может сохранить данные в очередь на сайте:
...
`payload` — тело исходного запроса.  
`endpoint` — endpoint, на который нужно будет переотправить (`/api/1c/products` по умолчанию).

Сайт раз в минуту обрабатывает очередь командой:

```bash
php artisan 1c:process-queue
```

После успешной обработки запись удаляется. Если обработка не удалась, счётчик попыток увеличивается. После 10 неудач запись считается "мертвой" и требует ручного разбора.
...
## Импорт файловой выгрузки CommerceML

Помимо HTTP-обмена поддерживается импорт готовых XML-файлов CommerceML 3.1, которые 1С записывает на диск.

### Формат файлов

В папке выгрузки ожидаются подпапки:

```text
goods/          карточки товаров
groups/         иерархия категорий
...
```bash
cd /path/to/api
php artisan 1c:import-commerceml /path/to/ВыгрузкаДляБота --apply
```

Флаги:

- `--apply` — сразу применить данные к каталогу (через staging + очередь).
- `--output=/tmp/export.json` — только сконвертировать в JSON, не писать в БД.
- `--chunk=500` — размер пачки при применении (по умолчанию 1000).

### Пример для Beget shared

После деплоя репозитория файлы выгрузки доступны в `docs/ВыгрузкаДляБота/`:

```bash
cd ~/gbsale.ru/api
php artisan 1c:import-commerceml ../docs/ВыгрузкаДляБота --apply --chunk=500
```

> Если очереди не запущены (`queue:work`), bulk-импорт останется в staging-таблицах. На shared-хостинге настройте cron `schedule:run` раз в минуту — тогда `Apply1CStagingData` обработается по расписанию.

### Ограничения

- Файл выгрузки `docs/ВыгрузкаДляБота/` содержит `СодержитТолькоИзменения="true"`, поэтому полноценное наполнение каталога возможно только после **полной выгрузки** из 1С.
- При импорте изменений товары, которых нет в файле, **не** помечаются неактивными автоматически.
...
На shared-хостинге вместо постоянного queue worker настройте cron на запуск раз в минуту:

```bash
* * * * * cd /path/to/api && php artisan schedule:run >> /dev/null 2>&1
```

В `routes/console.php` уже настроены:

- `php artisan bot:rebuild-index` — каждый час.
- `php artisan 1c:process-queue` — каждую минуту.
...
Готовый модуль для 1С:УНФ 3.0 лежит в `docs/1C_PROCESSING_MODULE.bsl`. Он содержит:
- отправку товара, цены, остатка, категории;
- автоматический fallback в очередь `/api/1c/queue` при сетевых/серверных ошибках;
- массовую bulk-выгрузку через `/api/1c/bulk-sync`.

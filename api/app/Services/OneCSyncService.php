...
class OneCSyncService
{
    ...
    public function ensureProductAndOffer(string $externalId, ?string $name = null): Offer
    {
        $product = Product::where('uuid_1c', $externalId)->first();

        if (! $product) {
            $name = $name ?? 'Import from 1C: ' . $externalId;
        ...
        return $this->ensureDefaultOfferForProduct($product, null, $product->name);
    }

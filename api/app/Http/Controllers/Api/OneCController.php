...
class OneCController extends Controller
{
    ...
    {
        $request->validate([
            'file' => ['required', 'file', 'mimes:json,txt', 'max:512000'],
        ]);
        ...
        if (! is_array($data)) {
            return response()->json([
                'success' => false,
                'message' => 'Файл должен содержать валидный JSON-объект.',
            ], 422);
        }
        ...
        return response()->json([
            'success' => true,
            'batch_id' => $batchId,
            'message' => 'Файл принят в обработку. Данные обновятся без перезатирания существующих записей.',
            'statistics' => $this->stagingService->getStatistics($batchId),
        ]);
    }
    ...
    public function syncQueue(Request $request): JsonResponse
    {
        ...
        Failed1CExport::create([
            'payload' => $data['payload'],
            'endpoint' => $data['endpoint'] ?? '/api/1c/products',
        ]);
    ...
    public function notifyPriceChanged(Request $request): JsonResponse
    {
        ...
        // Вернуть NotifyPriceChangedTo1C::dispatch(...) при настройке вебхука.

        return response()->json([
            'success' => true,
            'message' => 'Уведомления в 1С временно отключены.',
        ]);
    }
